 ...
 * method is much simpler than the one used by the <code>StreamTokenizer</code> class. The <code>
 * StringTokenizer</code> methods do not distinguish among identifiers, numbers, and quoted strings,
 * nor do they recognize and skip comments.
 *
 * <p>The set of delimiters (the characters that separate tokens) may be specified either at
 * creation time or on a per-token basis.
 *
 * <p>An instance of <code>StringTokenizer</code> behaves in one of two ways, depending on whether
 * it was created with the <code>returnDelims</code> flag having the value <code>true</code> or
 * <code>false</code>:
 *
 * <ul>
 *   <li>If the flag is <code>false</code>, delimiter characters serve to separate tokens. A token
 *       is a maximal sequence of consecutive characters that are not delimiters.
 *   <li>If the flag is <code>true</code>, delimiter characters are themselves considered to be
 *       tokens. A token is thus either one delimiter character, or a maximal sequence of
 *       consecutive characters that are not delimiters.
 * </ul>
 *
 * <p>A <tt>StringTokenizer</tt> object internally maintains a current position within the string to
 * be tokenized. Some operations advance this current position past the characters processed.
 *
 ...
 * </blockquote>
 *
 * <p><tt>StringTokenizer</tt> is a legacy class that is retained for compatibility reasons although
 * its use is discouraged in new code. It is recommended that anyone seeking this functionality use
 * the <tt>split</tt> method of <tt>String</tt> or the java.util.regex package instead.
 *
 * <p>The following example illustrates how the <tt>String.split</tt> method can be used to break up
 * a string into its basic tokens:
 *
 * <blockquote>
 *
 * <pre>
...
public class StringTokenizer implements Enumeration<Object> {
   ...
   * </code> argument are the delimiters for separating tokens. Delimiter characters themselves will
