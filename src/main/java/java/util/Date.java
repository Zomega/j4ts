...
public class Date implements Cloneable, Comparable<Date>, Serializable {
  ...
  private static def.js.Object jsdateClass() {
  ...
  public String toGMTString() {
    return $apply(jsdate.$get("getUTCDate"), jsdate)
        ...
        + ":"
        + padTwo((int) $apply(jsdate.$get("getUTCSeconds"), jsdate))
        + " GMT";
  }
  ...
  public String toString() {
    ...
    return StringData.DAYS[(int) getDay()]
        + " "
        + StringData.MONTHS[(int) getMonth()]
        + " "
        + padTwo((int) getDate())
        + " "
        + padTwo((int) getHours())
        + ":"
        + padTwo((int) getMinutes())
        + ":"
        + padTwo((int) getSeconds())
        + " GMT"
        + hourOffset
        + minuteOffset
        + " "
  ...
  private void fixDaylightSavings(int requestedHours) {
    ...
    if (getHours() != requestedHours) {
      ...
      int timeDiff =
          (int) $apply(jsdate.$get("getTimezoneOffset"), jsdate)
      ...
      if (timeDiff > 0) {
        ...
        def.js.Object newTime =
            $new(
                jsdateClass(),
                (int) $apply(jsdate.$get("getFullYear"), jsdate),
                getMonth(),
                day,
                requestedHours + timeDiffHours,
                getMinutes() + timeDiffMinutes,
                getSeconds(),
