...
abstract class AbstractHashMap<K, V> extends AbstractMap<K, V> {
  ...
  private final class EntrySetIterator implements Iterator<Entry<K, V>> {
    private Iterator<Entry<K, V>> stringMapEntries;
    private Iterator<Entry<K, V>> current;
    private Iterator<Entry<K, V>> last;
    private boolean hasNext;

    public EntrySetIterator() {
      stringMapEntries = stringMap.iterator();
      current = stringMapEntries;
  ...
  public AbstractHashMap(int ignored, float alsoIgnored) {
    // This implementation of HashMap has no need of load factors or
    // capacities.
  ...
  public boolean containsValue(Object value) {
    return _containsValue(value, stringMap) || _containsValue(value, hashCodeMap);
  }

  private boolean _containsValue(Object value, Iterable<Entry<K, V>> entries) {
    for (Entry<K, V> entry : entries) {
      if (_equals(value, entry.getValue())) {
        return true;
      }
    }
    return false;
  ...
  public V get(Object key) {
    V v =
        key instanceof String ? getStringValue(JsUtils.unsafeCastToString(key)) : getHashValue(key);
  ...
  @Override
  public int size() {
    return hashCodeMap.size() + stringMap.getSize();
  }

  /** Subclasses must override to return a whether or not two keys or values are equal. */
  abstract boolean _equals(Object value1, Object value2);
