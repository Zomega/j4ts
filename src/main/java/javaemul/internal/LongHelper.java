...
public final class LongHelper extends NumberHelper implements Comparable<LongHelper> {
  ...
  public static long reverseBytes(long i) {
    int high = (int) (i >>> 32);
    int low = (int) i;
    return ((long) IntegerHelper.reverseBytes(low) << 32)
        | (IntegerHelper.reverseBytes(high) & 0xffffffffL);
  }
  ...
  public static String toString(long value, int intRadix) {
    if (intRadix == 10
        || intRadix < CharacterHelper.MIN_RADIX
        || intRadix > CharacterHelper.MAX_RADIX) {
      return String.valueOf(value);
    }
