...
public final class CharacterHelper implements Comparable<CharacterHelper>, Serializable {
  ...
  public static int codePointAt(char[] a, int index, int limit) {
    ...
    char loSurrogate;
    if (CharacterHelper.isHighSurrogate(hiSurrogate)
        && index < limit
  ...
  public static int codePointBefore(char[] a, int index, int start) {
    ...
    if (isLowSurrogate(loSurrogate)
        && index > start
        && isHighSurrogate(highSurrogate = a[index - 1])) {
  ...
  public static boolean isLetterOrDigit(char c) {
    ...
    if (array != null) return string(String.valueOf(c)).match(leterOrDigitRegex()).length > 0;
    return false;
  }
  ...
  public static char[] toChars(int codePoint) {
    checkCriticalArgument(
        ...
        "CodePoint %s not in range [%s, %s]",
        Integer.toString(codePoint),
        "0",
        Integer.toString(MAX_CODE_POINT));

    if (codePoint >= MIN_SUPPLEMENTARY_CODE_POINT) {
      return new char[] {
        getHighSurrogate(codePoint), getLowSurrogate(codePoint),
      };
    } else {
      return new char[] {
        (char) codePoint,
